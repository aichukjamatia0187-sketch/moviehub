...
async function openDetails(id,type){try{const m=await api(`/api/${type==='tv'?'tv':'movie'}/${id}`);const trailer=(m.videos?.results||[]).find(v=>v.site==='YouTube'&&v.type==='Trailer')||(m.videos?.results||[]).find(v=>v.site==='YouTube');const cast=(m.credits?.cast||[]).slice(0,8);const tv=type==='tv';const title=tv?m.name:m.title;const date=tv?m.first_air_date:m.release_date;const runtime=tv?(m.episode_run_time?.[0]?m.episode_run_time[0]+' min/episode':'Runtime N/A'):(m.runtime?m.runtime+' min':'Runtime N/A');$('#modalBox').innerHTML=`<div class="details-hero" style="background-image:url('${backdrop(m)}')"><div class="details-content"><div class="eyebrow">${tv?'TV SHOW / DRAMA':'FILM'} DETAILS</div><h2>${esc(title)}</h2><div class="meta"><span class="score">${m.vote_average?.toFixed(1)||'—'}</span><span>★</span><span>${(date||'').slice(0,4)}</span><span>${runtime}</span><span>${tv&&m.number_of_seasons?m.number_of_seasons+' season'+(m.number_of_seasons>1?'s':''):''}</span></div><p class="overview">${esc(m.overview||'No synopsis available.')}</p><div class="actions">${trailer?`<button class="btn primary" onclick="playTrailer('${trailer.key}')">WATCH TRAILER</button>`:''}</div></div></div><div class="details-body"><div><h3>Cast</h3><div class="credits">${cast.map(p=>`<div class="person"><img src="${p.profile_path?IMG+'w185'+p.profile_path:''}" alt="${esc(p.name)}"><span>${esc(p.name)}</span></div>`).join('')}</div></div><div class="facts"><div><b>Genres</b><br>${(m.genres||[]).map(g=>esc(g.name)).join(' · ')||'—'}</div><div><b>Original title</b><br>${esc(tv?m.original_name||'—':m.original_title||'—')}</div><div><b>First release</b><br>${esc(date||'—')}</div><div><b>Language</b><br>${esc(m.original_language||'—').toUpperCase()}</div>${tv?`<div><b>Episodes</b><br>${m.number_of_episodes||'—'}</div>`:''}</div></div>`;$('#modal').classList.add('open')}catch(e){toast(e.message)}}
