...
async function api(path, params={}){
  ...
  if (!r.ok || d.error) throw new Error(d.status_message || d.error || 'Request failed');
  return d;
}
...
function fill(id,arr,type){
  const el = $(id);
  if(!el) return;
  el.innerHTML=arr.filter(x=>x.poster_path).slice(0,14).map(x=>card(x,type)).join('')||'<div class="empty">No titles found</div>';
}
...
function setHero(m){
  ...
  if($('#heroBg')) $('#heroBg').style.backgroundImage=`url("${backdrop(m)}")`;
...
async function load(){
  try{
    const [t,p,n,u,g,tp,tt,ta,tg]=await Promise.all([
      ...
      api('/movie/popular', { region: 'IN' }),
      api('/movie/now_playing', { region: 'IN' }),
      api('/movie/upcoming', { region: 'IN' }),
      api('/genre/movie/list'),
      api('/tv/popular', { region: 'IN' }),
      api('/trending/tv/day'),
      api('/tv/airing_today', { region: 'IN' }),
      api('/genre/tv/list')
    ]);
    ...
    toast('TMDB connection error: '+e.message);
  }
}
...
async function openDetails(id,type){
  try{
    ...
    const title=tv?m.name:m.title;
    const date=tv?m.first_air_date:m.release_date;
    const runtime=tv?(m.episode_run_time?.[0]?m.episode_run_time[0]+' min/episode':'Runtime N/A'):(m.runtime?m.runtime+' min':'Runtime N/A');

    if($('#modalBox')) {
      $('#modalBox').innerHTML=`<div class="details-hero" style="background-image:url('${backdrop(m)}')"><div class="details-content"><div class="eyebrow">${tv?'TV SHOW / DRAMA':'FILM'} DETAILS</div><h2>${esc(title)}</h2><div class="meta"><span class="score">${m.vote_average?.toFixed(1)||'—'}</span><span>★</span><span>${(date||'').slice(0,4)}</span><span>${runtime}</span><span>${tv&&m.number_of_seasons?m.number_of_seasons+' season'+(m.number_of_seasons>1?'s':''):''}</span></div><p class="overview">${esc(m.overview||'No synopsis available.')}</p><div class="actions">${trailer?`<button class="btn primary" onclick="playTrailer('${trailer.key}')">WATCH TRAILER</button>`:''}</div></div></div><div class="details-body"><div><h3>Cast</h3><div class="credits">${cast.map(p=>`<div class="person"><img src="${p.profile_path?IMG+'w185'+p.profile_path:''}" alt="${esc(p.name)}"><span>${esc(p.name)}</span></div>`).join('')}</div></div><div class="facts"><div><b>Genres</b><br>${(m.genres||[]).map(g=>esc(g.name)).join(' · ')||'—'}</div><div><b>Original title</b><br>${esc(tv?m.original_name||'—':m.original_title||'—')}</div><div><b>First release</b><br>${esc(date||'—')}</div><div><b>Language</b><br>${esc(m.original_language||'—').toUpperCase()}</div>${tv?`<div><b>Episodes</b><br>${m.number_of_episodes||'—'}</div>`:''}</div></div>`;
    }
...
function playTrailer(key){
  if($('#modalBox')) $('#modalBox').innerHTML=`<div class="video"><iframe src="https://www.youtube.com/embed/${encodeURIComponent(key)}?rel=0" title="Trailer" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe></div>`;
}
...
async function heroAction(){
  ...
  try{
    ...
    }else toast('No trailer is listed for this title.');
  }catch(e){
    toast(e.message);
  }
}
...
if($('#genresList')) $('#genresList').onclick=e=>{const b=e.target.closest('.genre');if(b) location.hash='genre-'+b.dataset.genre};
